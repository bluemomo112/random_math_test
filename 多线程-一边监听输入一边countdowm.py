...
def get_input(right_ans):
    t=turtle.Screen()
    ans=t.numinput("请输入答案：")
    if ans==right_ans:
        print("回答正确")
    else:
        print("回答错误")
...
if __name__ == '__main__':
    ...
    for i in range(questions_time):
        ...
        user_ans=""
        while user_ans != str(right_ans) and p1.is_alive()==True>0:
            user_ans = input("请输入您的答案：")
            print(p1.is_alive())

        if user_ans==str(right_ans) and p1.is_alive()==True:
            score+=1
